...
function View:init(ui,t,p)
    p = p or {}
    self.baseRotation = qRotation(0,vec3(1,0,0))
    self.orientRotation = vec4(1,0,0,0)
    self.intScale = 1
    self.extScale = 1
    self.fishEye = 1
    self.speed = 1/600
...
function View:draw()
    self.orientRotation = qRotationRate() * self.orientRotation 
...
function View:saveVelocity(touch)
    ...
    if lp < .9 and lv < .9 then
        ...
        qg = self:getGravity()
        qa = qg * qa * qg^""
        self.angVelocity = qa:make_slerp(vec4(1,0,0,0))
